...
def get_charater_quotes(request, pk):
    r = requests.get(
        f"{url}character/{pk}/quote", headers={"Authorization": ONE_AUTH}
    ).json()
    data = r["docs"]
    if len(data) == 0:
...
def add_favorite(request, pk):
    ...
    if hasattr(character, "status_code"):
        ...
        favorite = FavoriteCharacter.objects.get_or_create(
...
def add_favorite_quote(request, pk_c, pk_q):
    ...
    if hasattr(character, "status_code"):
        ...
        if r.status_code == 500:
            ...
            favorite_quote = FavoriteQuote.objects.get_or_create(
                user=user, quote=quote[0]
            )
...
def check_or_get_character(pk_c):
    ...
    try:
        ...
        r_character = requests.get(
            f"{url}character/{pk_c}/", headers={"Authorization": ONE_AUTH}
        )
        if r_character.status_code == 500:
            return Response(
                {"message": f"Character with the id:{pk_c} does not exist"},
        ...
        if int(r_character.json()["total"]) == 0:
            ...
            character = Character.objects.create(
                ...
                death=data["death"].strip('"'),
                realm=data["realm"].strip('"'),
                hair=data["hair"].strip('"'),
                name=data["name"],
